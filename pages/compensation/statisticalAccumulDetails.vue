<template>
  <div class="yt-dome-container">
    ...
    <el-table-column
      prop="compPayMoney"
      :label="listMonth+'公司负担公积金'"
      align=right>
      <template scope="scope">
          <span >{{scope.row.compPayMoney | moneyFormat}}</span>
        </template>
    </el-table-column>
    <el-table-column
      prop="personalPayMoney"
      :label="listMonth+'公司个人负担公积金'"
      align=center>
      <template scope="scope">
          <span >{{scope.row.personalPayMoney | moneyFormat}}</span>
        </template>
    </el-table-column>
    <el-table-column
    prop="payCompName"
...
export default {
  ...
  getProjectList() {
    ...
    axios
      .post(
        option.base_path +
          "fundPayMonth/getFundPaySocialList",
        this.form
      )
      .then(res => {
  ...
  saveForm(formName) {
    ...
    beforeExport(){
          ...
          let	category = '1';
          let pageParam =  '&month=' + month+ '&affiliatedComp=' + affiliatedComp + '&category=' + category;
          window.location.href = option.base_path + 'fundPayMonth/exportFundPayStatisticsList?' + pageParam;
    },
