  ...
  <div class="dome-container">
    ...
    <el-row>
        <el-col :span="24" align='right'>
          <el-button type="primary" @click='pastcheck'>打印单据粘贴单</el-button>
        <el-button type="primary" @click='printEven' v-if="form2.deptName == '财务核算岗' " >打印支出凭单</el-button>
          <el-button type="primary" @click="mingcheck"  v-if="form2.deptName == '财务核算岗' " >打印支出凭单(明细)</el-button>
        </el-col>
    </el-row>
      ...
      <el-tabs type="border-card" v-model="activeTab" @tab-click="handleClick">
        ...
        <el-tab-pane label="待处理"  v-model="form.queryStateParams" name="first">
          <el-table
            ...
            style="width: 100%;">
            <el-table-column
              prop="expenditureAppNum"
              label="单据编号"
              width="140"
              align='center'
              >
              <template slot-scope='scope'>
                <span  style="color:#417095;cursor: pointer;" @click='toPage4("/expensesReim/expenseAppDetail",scope.row)'>{{scope.row.expenditureAppNum}}</span>
              </template>
              <!-- <el-link type="primary">{{pendingTableData.num}}</el-link> -->
            </el-table-column>
            <el-table-column
              prop="expenditureAppName"
              label="支出事由"
              width="">
            </el-table-column>
            <el-table-column
            ...
            <el-table-column
              ...
              <template slot-scope="sc">
                <el-link type="primary" v-show="sc.row.taskKey!='activitiStartTask'" @click='toPage2("/expensesReim/expenseAppDetail",sc.row)' >审批</el-link>
                <el-link type="primary" v-show="sc.row.taskKey=='activitiStartTask'" @click='toPage3("./expenditureApp",sc.row)' >处理</el-link>
  ...
  methods: {
    ...
    mingcheck(){
      if(this.currentRow != null){
            this.toPage('./printDetails',this.currentRow)
        }else{
...
//     },
    ...
    getMessage(){
      axios.post(option.base_path+'/sz/expenditureApp/getUserExpenditureAppInfoWFListToPageByParams',this.form).then(res => {
    ...
    getmessagetwo(){
      axios.post(option.base_path+'/index/getCurrentUserInfo',this.form2).then(res => {
        // console.log(this.data.data.rows)
    ...
    pastcheck(){
      if(this.currentRow != null){
            this.toPage('/expensesReim/documentSticker',this.currentRow)
        }else{
            this.$message('请选中一行数据进行操作');
        }
    },
    ...
      printEven(){
      if(this.currentRow != null){
            this.toPage('/expensesReim/print/expenditureVoucher',this.currentRow)
        }else{
            this.$message('请选中一行数据进行操作');
        }
    },
...
        path: path,
        query: {
          expenditureAppId: row.expenditureAppId,
          costType: row.costType
        }
      });
    },
